// Copyright(c) 2007-2009 David Whittinghill
...
int* getIndex(string numString) {
    //cout << "Index String: '" << numString << "'" << endl;
    ...
    for (int j = 0; j < 3; j++) {
        ...
        //cout << "numString: " << newElement[j] << endl;
        //cout << "numString: " << start << " " << end << endl;
    }
    return newElement;
}

float* getVertex(string numString) {
//    cout << "Vertex String: " << numString << endl;
    float *newElement = new float[3];
    for (int j = 0; j < 3; j++) {
        size_t start = 0, end = 0;
        end = numString.find(" ",start);
        string str = numString.substr(start,end);
        //newElement[j] = strtod(str.c_str(),NULL);
    ...
    for (int i = 0; i < numOfModels; i++) {
        ...
        if (readObjFile(string(argv[i+2]))) {
            ...
            if (outFile.is_open()) {
                ...
                //outFile << "ReverseOrientation" << endl;
                outFile << "AttributeBegin" << endl;
                //outFile << "Translate 0 -.033 0" << endl;
